...
{
    uint32_t movementLocation = row * TicTacToe::game::boardWidth + column;
...
void TicTacToe::move(const name &challenger, const name &host, const name &by, const uint16_t &row, const uint16_t &column)
{
    ...
    check(itr->winner == TicTacToe::none, "The game has ended.");
    ...
    existingHostGames.modify(itr, itr->host, [&](auto &g) {
        g.board[row * TicTacToe::game::boardWidth + column] = cellValue;
        g.turn = turn;
        g.winner = GetWinner(g);
    });
}

name GetWinner(const TicTacToe::game &currentGame)
{
    ...
    std::vector<uint32_t> consecutiveColumn(TicTacToe::game::boardWidth, 3);
    std::vector<uint32_t> consecutiveRow(TicTacToe::game::boardHeight, 3);
    ...
    for (uint32_t i = 0; i < board.size(); i++)
    {
        ...
        uint16_t row = uint16_t(i / TicTacToe::game::boardWidth);
        uint16_t column = uint16_t(i % TicTacToe::game::boardWidth);
        ...
        // Calculate consecutive diagonal / value
        if (row + column == TicTacToe::game::boardWidth - 1)
        {
    ...
    return isBoardFull ? TicTacToe::draw : TicTacToe::none;
}
